...
add_filter('drts_core_component_paths', function ($paths) {
    $paths['directories-frontend'] = [__DIR__ . '/lib/components', '1.3.108'];
    return $paths;
});
...
if (is_admin()) {
    ...
    add_action('in_plugin_update_message-directories-frontend/directories-frontend.php', function ($plugin, $response) {
        printf(
            ' ' . esc_html__('Make sure to bulk update all Directories plugins at once. See %s for more details.', 'directories-frontend'),
        ...
        if (($new_version_parts = explode('.', $response->new_version))
            ...
            echo '<br /><br /><strong>' . esc_html__('Attention!', 'directories-frontend') . '</strong> ' . esc_html__('The update available is a major update of the plugin. Please be sure to backup your site before updating since there is no going back.', 'directories-frontend');
        }
        if ($pos = strpos($response->new_version, '-dev')) {
            printf(
                '<br /><br /><strong>' . esc_html__('Attention!', 'directories-frontend') . '</strong> ' . esc_html__('The update available is a development version of %s. If you do not feel comfortable to use a development version, please wait for the stable release.', 'directories-frontend'),
